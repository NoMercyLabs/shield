...
import { AlertTriangle, ExternalLink, GitBranch, Pencil, Play, ShieldCheck, Sparkles, Trash2, Upload, User, UserPlus, X } from 'lucide-vue-next'
...
async function onConfirmBulkApply(): Promise<void> {
  ...
  try {
    ...
    if (result.pullRequestUrl) {
      push('success', t('source_detail.bulk_apply_pr_opened', { url: result.pullRequestUrl }))
    }
    else {
      ...
      >
        <div class="w-full max-w-lg rounded-lg border border-slate-700 bg-slate-900 shadow-xl">
          ...
          <div class="max-h-96 overflow-y-auto px-5 py-4 text-sm">
            <p v-if="bulkPreviewResult.entries.length === 0 && bulkPreviewResult.errors.length === 0" class="text-slate-400">
          ...
          <footer class="flex justify-end gap-2 border-t border-slate-800 px-5 py-3">
            ...
            <button
              type="button"
              :disabled="bulkApplying || bulkPreviewResult.entries.length === 0"
