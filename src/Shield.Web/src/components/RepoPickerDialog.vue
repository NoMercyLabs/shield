...
import { useGitHubReposQuery, useProviderReposQuery } from '@/queries/oauth'
import { enumLabelByName } from '@/stores/enums'
import type { BulkSelection, OAuthProviderName, RepositorySummary } from '@/types/api'
...
const githubEnabled = computed(() => props.open && isGithub.value)
const genericEnabled = computed(() => props.open && !isGithub.value)

const githubQuery = useGitHubReposQuery(githubEnabled)
const genericQuery = useProviderReposQuery(computed(() => props.provider), genericEnabled)

const isLoading = computed(() => isGithub.value ? githubQuery.isLoading.value : genericQuery.isLoading.value)
const isError = computed(() => isGithub.value ? githubQuery.isError.value : genericQuery.isError.value)
const error = computed(() => isGithub.value ? githubQuery.error.value : genericQuery.error.value)

const rawRepos = computed<RepositorySummary[]>(() => {
  if (isGithub.value) {
    return (githubQuery.data.value?.repos ?? []).map(repo => ({
      owner: repo.owner,
      name: repo.name,
  ...
  return genericQuery.data.value?.repos ?? []
})

const totalCount = computed<number>(() => {
  if (isGithub.value) return githubQuery.data.value?.total ?? 0
  return genericQuery.data.value?.total ?? 0
})

const providerLabel = computed(() => enumLabelByName('OAuthProvider', props.provider) || props.provider)
...
const groupedByOwner = computed<{ owner: string, repos: RepositorySummary[] }[]>(() => {
...
const allVisibleSelected = computed(() => {
  ...
  return visible.every(repo => selected.value.has(repo.fullName))
})

const someVisibleSelected = computed(() => {
  return filteredRepos.value.some(repo => selected.value.has(repo.fullName))
})

function toggleRepo(repo: RepositorySummary): void {
...
function ownerAllSelected(owner: string): boolean {
  ...
  return repos.length > 0 && repos.every(repo => selected.value.has(repo.fullName))
}

function toggleOwnerAll(owner: string): void {
  const repos = groupedByOwner.value.find(group => group.owner === owner)?.repos ?? []
  const next = new Set(selected.value)
  const allSelected = repos.every(repo => next.has(repo.fullName))
  if (allSelected) repos.forEach(repo => next.delete(repo.fullName))
  else repos.forEach(repo => next.add(repo.fullName))
  selected.value = next
}
...
function toggleAllVisible(): void {
  const visible = filteredRepos.value
  const next = new Set(selected.value)
  const allSelected = visible.every(repo => next.has(repo.fullName))
  if (allSelected) visible.forEach(repo => next.delete(repo.fullName))
  else visible.forEach(repo => next.add(repo.fullName))
  selected.value = next
}
...
function onSubmit(): void {
  ...
  const selections: BulkSelection[] = rawRepos.value
    .filter(repo => selected.value.has(repo.fullName))
    .map(repo => ({
      owner: repo.owner,
      name: repo.name,
    ...
    >
      <div class="flex w-full max-w-3xl flex-col rounded-lg border border-slate-800 bg-slate-950 text-slate-100 shadow-2xl" style="max-height: 80vh">
        <header class="flex items-center justify-between border-b border-slate-800 px-4 py-3">
          <div>
            <h2 class="text-lg font-semibold">{{ t('repo_picker.title_generic', { provider: providerLabel }) }}</h2>
        ...
        <div class="flex flex-wrap items-center gap-3 border-b border-slate-800 px-4 py-2 text-sm">
          ...
          <p class="ml-auto text-xs text-slate-500">{{ t('repo_picker.showing_of', { shown: totalShown, total: totalCount }) }}</p>
        </div>
        ...
        <div class="min-h-0 flex-1 overflow-y-auto">
          ...
          <p v-else-if="!groupedByOwner.length" class="p-4 text-sm text-slate-500">
            ...
            <span v-else>{{ t('repo_picker.empty_generic', { provider: providerLabel }) }}</span>
          </p>
          <div v-else>
            <section
              ...
              <ul class="divide-y divide-slate-800/40">
                ...
                >
                  <label
                    ...
                    :class="{ 'border-blue-500 bg-blue-600': selected.has(repo.fullName) }"
                    @click.stop
                  >
                    <input
                      type="checkbox"
                    ...
                    />
                    <Check v-if="selected.has(repo.fullName)" class="h-3.5 w-3.5 text-white" />
                  </label>
                  ...
                  <span v-if="repo.isPrivate" class="inline-flex items-center gap-1 rounded border border-amber-700 bg-amber-950 px-1.5 py-0.5 text-xs text-amber-300">
                    <Lock class="h-3 w-3" />
        ...
        <footer class="flex items-center justify-between border-t border-slate-800 px-4 py-3">
          <p class="text-sm text-slate-400">
            ...
            <span v-if="totalCount && totalCount !== totalShown" class="text-slate-500">
              {{ t('repo_picker.selected_of_total', { total: totalCount }) }}
            </span>
          </p>
