...
const serializeFindingFilter = (filter: FindingFilter): URLSearchParams => {
  ...
  if (filter.hasFix != null)
    params.append('hasFix', String(filter.hasFix))
  if (filter.kevOnly === true)
    params.append('kevOnly', 'true')
  if (filter.epssMin != null && filter.epssMin > 0)
    params.append('epssMin', String(filter.epssMin))
  if (filter.advisoryQuery)
    params.append('advisoryQuery', filter.advisoryQuery)
  if (filter.sortBy)
