...
public sealed class PrCommentService : IPrCommentService
{
    ...
    )
    {
        ...
        if (source is null)
            return new PrCommentResult(false, 0, 0, null, "no matching Source");

        IScanner? scanner = _scannerRegistry.FindFor(SourceType.GithubRepo);
        if (scanner is null)
            return new PrCommentResult(false, 0, 0, null, "no GithubRepo scanner registered");
        ...
        if (!prScan.Success || prScan.Snapshot is null)
            return new PrCommentResult(true, 0, 0, null, $"scan failed: {prScan.Error}");
        ...
        if (added.Count == 0)
            return new PrCommentResult(true, 0, 0, null, "no added dependencies");

        List<PrFinding> vulnerable = await MatchAddedAsync(added, ct);
        if (vulnerable.Count == 0)
            return new PrCommentResult(
                true,
                added.Count,
                0,
                null,
                "no advisories matched added deps"
            );
        ...
        try
        {
            ...
            return new PrCommentResult(true, added.Count, vulnerable.Count, commentId, null);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(
    ...
    )
    {
        ...
        )
        {
            ...
            {
                foreach (Advisory advisory in hits)
                {
                    if (
                        !string.Equals(
                        ...
                        continue;
                    FixSuggestion? fix = _fixSuggester.Suggest(advisory, item);
                    result.Add(new PrFinding(item, advisory, fix));
                }
            }
        }
        return result;
...
// can swap in a substitute IGitHubClient without spinning up an HTTP server.
...
public sealed class GitHubClientFactory : IGitHubClientFactory
{
    ...
    public IGitHubClient Create(string accessToken) =>
        new GitHubClient(ProductHeader) { Credentials = new Credentials(accessToken) };
}
