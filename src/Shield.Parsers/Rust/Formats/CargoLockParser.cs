...
internal static class CargoLockParser
{
    public static ParseResult Parse(string text)
    {
        ...
        foreach (CargoPackage pkg in packages)
        {
            ...
            // Cargo.lock doesn't encode direct/transitive on its own — Cargo.toml is needed
            // for that distinction. Default true to preserve backward-compat with the migration
            // default; CargoTomlParser emits a separate manifest-only pass with IsDirect=true.
            items.Add(
                new InventoryItem
                {
