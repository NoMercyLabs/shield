...
public sealed class NugetLockParser : IParser
{
    ...
    )
    {
        ...
        if (IsCentralPackagesProps(filename))
            return await ParseCentralPackagesPropsAsync(content, ct).ConfigureAwait(false);
    ...
    // Matches Directory.Packages.props and Directory.Packages.<env>.props (any casing).
    static bool IsCentralPackagesProps(string filename)
    {
        string name = Path.GetFileName(filename);
        if (!name.EndsWith(".props", StringComparison.OrdinalIgnoreCase))
            return false;
        string stem = name[..^".props".Length];
        // Accept "Directory.Packages" and "Directory.Packages.<anything>"
        if (stem.Equals("Directory.Packages", StringComparison.OrdinalIgnoreCase))
            return true;
        if (
            stem.StartsWith("Directory.Packages.", StringComparison.OrdinalIgnoreCase)
    ...
    static async ValueTask<ParseResult> ParseCentralPackagesPropsAsync(
        Stream content,
        CancellationToken ct
    )
    {
        ...
        MatchCollection packageVersionMatches = Regex.Matches(
            text,
            "<(?:PackageVersion|GlobalPackageReference)\\s+[^>]*Include\\s*=\\s*\"(?<name>[^\"]+)\"[^>]*Version\\s*=\\s*\"(?<version>[^\"]+)\"",
            RegexOptions.IgnoreCase
        );

        foreach (Match match in packageVersionMatches)
        {
    ...
    static async ValueTask<ParseResult> ParseCsprojAsync(Stream content, CancellationToken ct)
    {
        ...
        // Only emit when a Version attribute is present on the same tag.
        // CPM csproj files have <PackageReference Include="X" /> with no Version;
        // those are skipped here — the central Directory.Packages.props is the source of truth.
