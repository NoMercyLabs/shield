...
public class GoSumParserTests
{
    ...
    public async Task ParseAsync_GoSum_DedupesPerPair()
    {
        ...
        result.Items.Should().ContainSingle(item => item.Name == "github.com/gin-gonic/gin" && item.Version == "v1.9.1");
        result.Items.Should().ContainSingle(item => item.Name == "github.com/stretchr/testify" && item.Version == "v1.8.4");
        result.Items.Should().ContainSingle(item => item.Name == "golang.org/x/sys" && item.Version == "v0.15.0");
    }
}
