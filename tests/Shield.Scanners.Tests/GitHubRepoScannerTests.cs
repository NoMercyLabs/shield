...
public class GitHubRepoScannerTests
{
    ...
    public async Task Scans_repo_tree_and_parses_recognized_blob()
    {
        ...
        TreeItem lockfileItem = BuildTreeItem(
            "apps/web/package-lock.json",
            "blob-sha-1",
            TreeType.Blob
        );
        TreeItem readmeItem = BuildTreeItem("README.md", "blob-sha-2", TreeType.Blob);
        TreeResponse tree = BuildTreeResponse("tree-sha", [lockfileItem, readmeItem]);
    ...
    private static Repository BuildRepo(long id) =>
        new(
    ...
    private static Reference BuildReference(string @ref, string sha)
    {
        TagObject obj = new(
            nodeId: "node-tag",
    ...
    private static TreeResponse BuildTreeResponse(string sha, IReadOnlyList<TreeItem> items) =>
        new(sha, $"https://api.github.com/repos/x/y/git/trees/{sha}", items, false);

    private static Blob BuildBlob(string content, EncodingType encoding, string sha) =>
        new(
            nodeId: "node-blob",
            content: content,
            encoding: encoding,
