...
public class PipfileLockParserTests
{
    ...
    public async Task ParseAsync_PipfileLock_StripsEqualsAndReturnsBothSections()
    {
        ...
        result.Items.Should().ContainSingle(item => item.Name == "flask" && item.Version == "3.0.0");
        result.Items.Should().ContainSingle(item => item.Name == "werkzeug" && item.Version == "3.0.1");
        result.Items.Should().ContainSingle(item => item.Name == "pytest" && item.Version == "8.0.0");
    }
}
