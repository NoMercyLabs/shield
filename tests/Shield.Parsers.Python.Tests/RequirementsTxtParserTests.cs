...
public class RequirementsTxtParserTests
{
    ...
    public async Task ParseAsync_RequirementsTxt_PinnedAndUnpinned()
    {
        ...
        result
            .Items.Should()
            .ContainSingle(item => item.Name == "django" && item.Version == "5.0.1");
        result
            .Items.Should()
            .ContainSingle(item => item.Name == "requests" && item.Version == "2.31.0");
        result
            .Items.Should()
            .ContainSingle(item => item.Name == "flask" && item.Version == string.Empty);
        result
            .Items.Should()
            .ContainSingle(item => item.Name == "pytest" && item.Version == string.Empty);
        result
            .Items.Should()
            .ContainSingle(item => item.Name == "black" && item.Version == string.Empty);
